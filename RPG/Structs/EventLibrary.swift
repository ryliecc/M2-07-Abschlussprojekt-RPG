...
struct EventLibrary {
    static let allEvents = [
        GameEvent(name: "Wandering Merchant",
                  ...
                  effect: { party in party.coins += 30; print("You gain 30 coins.") }),
        ...
        GameEvent(name: "Broken Cart",
                  ...
                  effect: { party in for hero in party.members { hero.healthPoints = min(hero.maxHealthPoints, hero.healthPoints + 5) }; print("Everyone gains up to 5 HP.") }),

        GameEvent(name: "Ambush!",
                  description: "Bandits spring from the bushes and steal some of your gold!",
                  rarity: .rare, time: .day,
                  effect: { party in party.coins = max(0, party.coins - 40); print("You lose \(party.coins > 0 ? "40" : "all") coins!") }),

        GameEvent(name: "Flower Meadow",
                  description: "You rest in a field of fragrant flowers. The scent soothes your soul.",
                  rarity: .uncommon, time: .day,
                  effect: { party in for hero in party.members { hero.manaPoints = min(hero.maxManaPoints, hero.manaPoints + 3) }; print("Everyone recovers 3 mana.") }),
...
//                 effect: { _ in print("This will affect the next area... (placeholder effect)") }),
        ...
        GameEvent(name: "Wolves Howling",
                  ...
                  effect: { party in for hero in party.members { hero.manaPoints = max(0, hero.manaPoints - 2) }; print("Everyone loses 2 mana.") }),
        ...
        GameEvent(name: "Nightmare Vision",
                  ...
                  effect: { party in for hero in party.members { hero.healthPoints = max(1, hero.healthPoints - 4) }; print("Each hero loses 4 HP.") }),

        GameEvent(name: "Shooting Star",
                  description: "You wish upon a shooting star. Magic fills the air.",
                  rarity: .rare, time: .night,
                  effect: { party in party.coins += 100; print("You mysteriously gain 100 coins!") }),

        GameEvent(name: "Midnight Visitor",
                  description: "A cloaked figure offers you a gift... but says nothing.",
                  rarity: .uncommon, time: .night,
                  effect: { party in for hero in party.members { hero.attackPower += 1 }; print("Everyone gains +1 attack power permanently.") }),
        
        GameEvent(name: "Ancient Totem",
                  description: "You find an ancient totem pulsing with strange energy.",
                  rarity: .rare, time: .both,
                  effect: { party in for hero in party.members { hero.defense += 2 }; print("All heroes gain +2 defense permanently!") }),
